...
pub mod prelude {
    ...
    pub use crate::colour::gradient::{
        IntoGradient, IntoGradientHsl, IntoGradientLch, IntoGradientOklch,
    };
...
mod test {
    ...
    fn get_image() -> UtilResult<DynamicImage> {
        ...
        let image = if let Some(max_dim) = MAX_DIM {
            ...
            if max_dim < x.max(y) as usize {
                let image = &image;
                let factor = max_dim as f32 / x.max(y) as f32;
    ...
    fn dither(
        ...
        if !only_custom {
            ...
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Bayer(2)))
                .save(format!("data/dither/bayer-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Bayer(4)))
                .save(format!("data/dither/bayer-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Bayer(8)))
                .save(format!("data/dither/bayer-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Bayer(16)))
                .save(format!("data/dither/bayer-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Diamonds(8)))
                .save(format!("data/dither/diamonds-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Diamonds(12)))
                .save(format!("data/dither/diamonds-12x12{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Diamonds(16)))
                .save(format!("data/dither/diamonds-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), CheckeredDiamonds(8)))
                .save(format!("data/dither/checkered-diamonds-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), CheckeredDiamonds(12)))
                .save(format!("data/dither/checkered-diamonds-12x12{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), CheckeredDiamonds(16)))
                .save(format!("data/dither/checkered-diamonds-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiagonalTiles(2)))
                .save(format!("data/dither/diagonal-tiles-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiagonalTiles(4)))
                .save(format!("data/dither/diagonal-tiles-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiagonalTiles(8)))
                .save(format!("data/dither/diagonal-tiles-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiagonalTiles(16)))
                .save(format!("data/dither/diagonal-tiles-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), BouncingBowtie(2)))
                .save(format!("data/dither/bouncing-bowtie-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), BouncingBowtie(4)))
                .save(format!("data/dither/bouncing-bowtie-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), BouncingBowtie(8)))
                .save(format!("data/dither/bouncing-bowtie-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), BouncingBowtie(16)))
                .save(format!("data/dither/bouncing-bowtie-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(2, Orientation::Horizontal),
                ))
                .save(format!("data/dither/scanline-horizontal-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(4, Orientation::Horizontal),
                ))
                .save(format!("data/dither/scanline-horizontal-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(8, Orientation::Horizontal),
                ))
                .save(format!("data/dither/scanline-horizontal-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(16, Orientation::Horizontal),
                ))
                .save(format!(
                    "data/dither/scanline-horizontal-16x16{postfix}.png"
                ))?;
            ...
            image
                ...
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(2, Orientation::Vertical),
                ))
                .save(format!("data/dither/scanline-vertical-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(4, Orientation::Vertical),
                ))
                .save(format!("data/dither/scanline-vertical-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(8, Orientation::Vertical),
                ))
                .save(format!("data/dither/scanline-vertical-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    ScanLine(16, Orientation::Vertical),
                ))
                .save(format!("data/dither/scanline-vertical-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Starburst(2)))
                .save(format!("data/dither/starburst-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Starburst(4)))
                .save(format!("data/dither/starburst-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Starburst(8)))
                .save(format!("data/dither/starburst-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Starburst(16)))
                .save(format!("data/dither/starburst-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), ShinyBowtie(2)))
                .save(format!("data/dither/shiny-bowtie-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), ShinyBowtie(4)))
                .save(format!("data/dither/shiny-bowtie-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), ShinyBowtie(8)))
                .save(format!("data/dither/shiny-bowtie-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), ShinyBowtie(16)))
                .save(format!("data/dither/shiny-bowtie-16x16{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), MarbleTile(2)))
                .save(format!("data/dither/marble-tile-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), MarbleTile(4)))
                .save(format!("data/dither/marble-tile-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), MarbleTile(8)))
                .save(format!("data/dither/marble-tile-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), MarbleTile(16)))
                .save(format!("data/dither/marble-tile-16x16{postfix}.png"))?;
            ...
            image
                ...
                .save(format!("data/dither/curve-path-2x2{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                ...
                .save(format!("data/dither/curve-path-4x4{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                ...
                .save(format!("data/dither/curve-path-8x8{postfix}.png"))?;
            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                ...
                .save(format!("data/dither/curve-path-16x16{postfix}.png"))?;
            ...
            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Stars))
                .save(format!("data/dither/stars{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), NewStars))
                .save(format!("data/dither/new-stars{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Grid))
                .save(format!("data/dither/grid{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Trail))
                .save(format!("data/dither/trail{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Crisscross))
                .save(format!("data/dither/crisscross{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Static))
                .save(format!("data/dither/static{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Wavy(Orientation::Vertical)))
                .save(format!("data/dither/wavy-vertical{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(
                    palette.clone(),
                    Wavy(Orientation::Horizontal),
                ))
                .save(format!("data/dither/wavy-horizontal{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), BootlegBayer))
                .save(format!("data/dither/bootleg-bayer{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Diagonals))
                .save(format!("data/dither/diagonals{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiagonalsBig))
                .save(format!("data/dither/diagonals-big{postfix}.png"))?;
            ...
            image
                ...
                .apply(&Ordered::new(
                    ...
                    DiagonalsN {
                        n: 8,
                        direction: DiagonalDirection::DownRight,
                        increase: Increase::Linear(1),
                    },
                ))
                .save(format!("data/dither/diagonals-n-dr+lin1-8x8{postfix}.png"))?;
            ...
            image
                ...
                .save(format!(
                    "data/dither/diagonals-n-dr+lin1-32x32{postfix}.png"
                ))?;
            ...
            image
                ...
                .apply(&Ordered::new(
                    ...
                    DiagonalsN {
                        n: 8,
                        direction: DiagonalDirection::DownRight,
                        increase: Increase::Exponential(2),
                    },
                ))
                .save(format!("data/dither/diagonals-n-dr+exp2-8x8{postfix}.png"))?;
            ...
            image
                ...
                .save(format!(
                    "data/dither/diagonals-n-dr+exp2-32x32{postfix}.png"
                ))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), DiamondGrid))
                .save(format!("data/dither/diamond-grid{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), SpeckleSquares))
                .save(format!("data/dither/speckle-squares{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), Scales))
                .save(format!("data/dither/scales{postfix}.png"))?;

            image
                .clone()
                .apply(&Ordered::new(palette.clone(), TrailScales))
                .save(format!("data/dither/trail-scales{postfix}.png"))?;
        }
        ...
        image
            ...
            .apply(&Ordered::new(
                palette.clone(),
                Custom(matrix)
                    .checker(dither::ordered::tools::properties::CheckerType::From {
                        source: dither::ordered::tools::properties::Source::Center,
                        factor: dither::ordered::tools::properties::Factor::Linear,
                        modulo: Some(4),
                    })
                    // .invert()
                    .mirror(MirrorLine {
                        direction: dither::ordered::tools::mirror::MirrorDirection::Upright,
                        flip: true,
                        thorough: true,
                    })
